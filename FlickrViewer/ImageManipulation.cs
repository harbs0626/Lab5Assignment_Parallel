...
namespace FlickrViewer
{
    ...
    {
        public ImageFormat SetImageFormat { get; set; }
        
        // ** Image Resizing
        public byte[] Resize(byte[] _imageData, int _imageWidth)
        {
            using (MemoryStream _memoryStream = new MemoryStream(_imageData))
            {
                ...
                {
                    _newThumbnail.Save(_myThumbnailStream, SetImageFormat);
                    return _myThumbnailStream.ToArray();
                }
            }
        }

        // ** NOT NEEDED - Image Conversion
